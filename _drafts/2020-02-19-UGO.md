...
No olvidemos que ahora mismo estos usuarios no pueden acceder a su cuenta ya que estas no tienen contraseña, para asignarles una contraseña utilizar el comando passwd ejemplo:    

```bash
passwd sistemas1    
```

Con estas configuracion los usuarios cumpliran los requisitos que se enumeran en el enunciado de este post.
